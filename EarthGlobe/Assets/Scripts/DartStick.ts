...
export class DartStick extends BaseScriptComponent {
  @input @hint("Sound when the dart sticks to a planet") @allowUndefined
  hitSound: AudioComponent = null
  ...
  private orbits: PlanetOrbits | null = null

  onAwake() {
    this.logger = new Logger("DartStick", this.enableLogging, true)
    this.bodyComponent = this.getSceneObject().getComponent("Physics.BodyComponent")
    if (!this.bodyComponent) {
      this.logger.error("Physics.BodyComponent is required on the dart!")
      return
    }
  ...
  private onCollisionEnter(e: CollisionEnterEventArgs) {
    ...
    const planet = hitObject.getComponent(PlanetInfo.getTypeName()) as PlanetInfo
    if (!planet) return // only react to planets

    // Stick: freeze and parent to the planet so it rides the orbit.
    ...
    if (orbits) orbits.showFact(planet.planetName, planet.getRandomFact())
  }

  // Find the PlanetOrbits in the scene once (the dart is a spawned prefab, can't pre-wire it).
  private getOrbits(): PlanetOrbits | null {
    if (this.orbits) return this.orbits
    const count = global.scene.getRootObjectsCount()
    for (let i = 0; i < count; i++) {
      const found = this.search(global.scene.getRootObject(i))
      if (found) { this.orbits = found; return found }
    }
    return null
  }

  private search(obj: SceneObject): PlanetOrbits | null {
    const c = obj.getComponent(PlanetOrbits.getTypeName()) as PlanetOrbits
    if (c) return c
    const n = obj.getChildrenCount()
    for (let i = 0; i < n; i++) {
      const f = this.search(obj.getChild(i))
      if (f) return f
    }
    return null
  }
